    ...
    loadTabIframe:function(jq,params){
        return jq.each(function(){
           ...
           $tabBody.css({'overflow':'hidden','position':'relative'});
            ...
            var $maskMessage = $('<div class="mask-message" style="z-index:3;width:auto;height:16px;line-height:16px;position:absolute;top:50%;left:50%;margin-top:-20px;margin-left:-92px;border:2px solid #4884FD;padding: 12px 5px 10px 30px;background: #ffffff url(\'/Statics/js/easyui/themes/default/images/loading.gif\') no-repeat scroll 5px center;">' + (params.iframe.message || '加载中,请稍后....') + '</div>').appendTo($tabBody);
